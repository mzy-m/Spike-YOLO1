...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, strides=[8, 16, 32]):
        ...
        self.qfl  =QFL(beta=2.0)
        self.asyloss=AsymmetricLoss(gamma_neg=2, gamma_pos=0, clip=0.03,  disable_torch_grad_focal_loss=True)
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_matched_classes, fg_mask, pred_iou_this_matching, matched_gt_inds, num_fg_img= self.get_assignments( 
                    num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, cls_preds_per_image, obj_preds_per_image,
                    expanded_strides, x_shifts, y_shifts,self.num_classes,
                )
                torch.cuda.empty_cache()
                num_fg      = num_fg +num_fg_img
                
                #cls_target  = F.one_hot(gt_matched_classes.to(torch.int64), self.num_classes).float()
                cls_target  = F.one_hot(gt_matched_classes.to(torch.int64), self.num_classes).float()*pred_iou_this_matching.unsqueeze(-1)
                ...
                obj_target=obj_target.type(cls_target.type())
        ...
        cls_targets_1=torch.zeros_like(cls_preds.view(-1, self.num_classes))
        cls_targets_1[fg_masks]=cls_targets
        
        
        
       
        loss_iou    = (self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)).sum()/ num_fg
        #loss_obj=(self.qfloss(obj_preds.view(-1, 1), obj_targets)).sum()/num_fg

       # loss_obj    = (self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)).sum()/ num_fg
        loss_obj    = (self.asyloss(obj_preds.view(-1, 1), obj_targets)).sum()/ num_fg
        
        loss_cls    = (self.bcewithlog_loss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()/ num_fg
        #loss_cls=(self.asyloss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()/num_fg
        #loss_cls    = (self.bcewithlog_loss(cls_preds.view(-1, self.num_classes), cls_targets_1)).sum()/ num_fg
        #loss_cls=(self.qfl(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()/num_fg
        #loss_cls=(self.qfloss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()/num_fg
    ...
    def qfloss(self,predicts, target, beta=4.0):
        
        predicts=predicts.sigmoid()
        scale_factor = (predicts - target).abs().pow(beta)
        # binary_cross_entropy_with_logits = sigmoid(preds) + binary_cross_entropy()
        
        loss =F.binary_cross_entropy(predicts, target, reduction='none')* scale_factor 
    ...
    @torch.no_grad()
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, cls_preds_per_image, obj_preds_per_image, expanded_strides, x_shifts, y_shifts,num_classes):
        #-------------------------------------------------------#
        ...
        #cls_iou=gt_cls_per_image*pair_wise_ious.unsqueeze(-1)
        
        pair_wise_cls_loss  = F.binary_cross_entropy(cls_preds_.sqrt_(), gt_cls_per_image, reduction="none").sum(-1)
        #pair_wise_cls_loss  =self.asyloss(cls_preds_.sqrt_(), gt_cls_per_image).sum(-1)
        #pair_wise_cls_loss=self.qfloss(cls_preds_.sqrt_(),cls_iou).sum(-1)
        ...
        del pair_wise_cls_loss, cost, pair_wise_ious_loss
